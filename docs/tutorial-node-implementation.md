...
- Python Archive: Execute the extracted Python script on the target. Python and Python dependencies, that are specified as  `requirements.txt` inside the archive, are ensured to be installed on the target.
